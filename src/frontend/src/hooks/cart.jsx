...
function CartProvider({ children }) {
    const [cart, setCart] = useState( JSON.parse(localStorage.getItem(`@cannoli:cart`)) || [])
    ...
    async function handleResetCart() {
        localStorage.removeItem(`@cannoli:cart`);
        setCart([]);
    }

    useEffect(() => {
        localStorage.setItem(`@cannoli:cart`, JSON.stringify(cart));
    }, [cart])
